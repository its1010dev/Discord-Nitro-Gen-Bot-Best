...
admins = "" #Put your ID Here
prefix = "prefix" #Prefix
nitro_timeout = 120  # Nitro Timeout
welcome = 867295294655430666 #Put Welcome Channel ID Here
bots_channel = 870980187834163210 #Put Bot Channel ID Here
drop_amount = 1000 # Dont change this
whitelist = [605111781370626069] # Whitelisted Users have no cooldown
...
async def nitro(ctx):
  if ctx.channel.id == bots_channel:
        x = 350
        bronze = discord.utils.get(ctx.guild.roles, name='「ᶦˢˡᵃⁿᵈ」G3n Acces')#250
        silver = discord.utils.get(ctx.guild.roles, name='「ᶦˢˡᵃⁿᵈ」Island')#200
        gold = discord.utils.get(ctx.guild.roles, name='「ᶦˢˡᵃⁿᵈ」Vizitatori Premium')#500
        premium = discord.utils.get(ctx.guild.roles, name='🤭')#1000
        ...
        while not (x == 0) :
            ...
            x -= 1
            a_file = open("k3ny_nitro.txt", "a" )
            a_file.write('K3ny ' + '\n' + 'discord.gift/'+ tokens + '\n')
            a_file.close()
        target = await bot.fetch_user(ctx.author.id)
        await target.send(file = discord.File(r'k3ny_nitro.txt'))
        os.remove("k3ny_nitro.txt")
        print(f'{ctx.author} | {ctx.author.id} -> {prefix}nitro')
        await target.send("Your Text Here")
